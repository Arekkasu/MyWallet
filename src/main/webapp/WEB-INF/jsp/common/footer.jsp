<%@ page contentType="text/html; charset=UTF-8" pageEncoding="UTF-8" %>
<footer id="contact">
    <div class="container-fluid text-center">
        <p class="p_contact">&copy; Arekkasu</p>
        <div class="d-flex justify-content-center">
            <a href="https://github.com/Arekkasu" class="social-link mx-2">
                <i class="social-icon fa-brands fa-github fa-xl icon-color"></i>
            </a>
            <a href="#" class="social-link mx-2">
                <i class="social-icon fa-brands fa-instagram fa-xl icon-color"></i>
            </a>
            <a href="mailto:alozadac@gmail.com" class="social-link mx-2">
                <i class="social-icon fa-solid fa-envelope fa-xl icon-color"></i>
            </a>
            <a href="https://www.linkedin.com/in/alexander-lozada-caviedes-470569264/" class="social-link mx-2">
                <i class="social-icon fa-brands fa-linkedin fa-xl icon-color"></i>
            </a>
        </div>
    </div>
</footer>